...
public class RechargeViewModel extends BaseViewModel<RechargeNavigator> {

    public RechargeViewModel(DataManager dataManager, SchedulerProvider schedulerProvider) {
    ...
    {
        getNavigator().onSelectOperator();
    }


    public void onRechargeButtonClick() {
        if(RechargeConfig.TYPE.equalsIgnoreCase(RechargeConfig.Type.PREPAID.toString()))
        getNavigator().onPrepaidRecharge();
        else if(RechargeConfig.TYPE.equalsIgnoreCase(RechargeConfig.Type.POSTPAID.toString()))
        getNavigator().onPostpaidRecharge();
